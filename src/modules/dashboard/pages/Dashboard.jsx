...
const Dashboard = () => {
  ...
  const stats = [
    {
      title: 'Tổng Số Dư',
      value: formatCurrencyShort(dashboardData?.totalBalance || 0),
      change: '+8.2%',
      icon: DollarSignIcon,
      color: 'text-blue-600',
    ...
    },
    {
      ...
      value: formatCurrencyShort(dashboardData?.monthlyIncome || 0),
      change: '+12.5%',
      icon: TrendingUpIcon,
    ...
    },
    {
      ...
      value: formatCurrencyShort(dashboardData?.monthlyExpense || 0),
      change: '-5.3%',
      icon: CreditCardIcon,
      color: 'text-red-600',
                ...
                <Card key={index} className="relative overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group min-w-0">
                  <CardContent className="p-2 sm:p-3 md:p-4">
                    <div className="text-center space-y-1 sm:space-y-2">
                      <div className={`p-1.5 sm:p-2 rounded-lg ${stat.bgColor} mx-auto w-fit group-hover:scale-110 transition-transform duration-300`}>
                        <Icon className={`w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 ${stat.color}`} />
                      </div>
                      <p className="text-[9px] xs:text-[10px] sm:text-xs font-medium text-gray-600 dark:text-gray-400 leading-tight overflow-hidden min-h-[2.4em]" style={{
                        display: '-webkit-box',
                        WebkitLineClamp: '2',
                      ...
                      <p className="text-xs xs:text-sm sm:text-base md:text-lg font-bold text-gray-900 dark:text-white leading-tight break-words min-h-[1.2em]">
                        {stat.value}
                      </p>
                      <div className="flex items-center justify-center space-x-0.5 sm:space-x-1 min-h-[1em]">
                        {stat.change.startsWith('+') ? (
                            <ArrowUpIcon className="w-2 h-2 xs:w-2.5 xs:h-2.5 sm:w-3 sm:h-3 text-green-600 flex-shrink-0" />
                        ) : (
                            <ArrowDownIcon className="w-2 h-2 xs:w-2.5 xs:h-2.5 sm:w-3 sm:h-3 text-red-600 flex-shrink-0" />
                        )}
                        <span className={`text-[8px] xs:text-[9px] sm:text-xs font-medium ${stat.changeColor} truncate max-w-[4rem] sm:max-w-none`}>
                          {stat.change}
                        </span>
                      </div>
                    </div>
                    ...
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600 dark:text-gray-400">{formatCurrencyShort(category.amount)}</p>
                        </div>
                      </div>
